...
**当前：** **2.0.1** 已发布（合入 1.6.0 / 1.6.1）。2.0.1 补充暂停后前台服务保留与进度/已听迁出 SharedPreferences；后续候选见 [ROADMAP.md](ROADMAP.md)。
...
- [x] User-Agent 为可识别的 `Chengbo/<version> (Flutter; chengbo radio)`，随 `AppBrand` / pubspec 对齐（当前 `2.0.1`）
...
- [x] 确认收听范围后并行检测直播源，列表只展示当前能连上的台；探测中可先听已测到的，可「停止检测」；二次打开沿用首次结果，设置「电台管理」里检测或改「收听范围」才再探测
...
- [x] **Android Auto / 车载**：MediaBrowser 浏览收藏 / 最近 / 电台 / 继续收听 / 已下载；通知栏与车机可上一台/下一台
...
- [x] Podcast Index 搜索 → 一键订阅（发现页高级；默认滤 explicit；密钥本机保存）
- [x] Chromecast（仅 Android；默认接收器 CC1AD845；无 Play Services 时静默失败）

### 2.0.1（2026-09-17）

- [x] 暂停播放后继续保留 Android 前台服务，通知可由用户移除
- [x] 播客进度与“已听”状态迁出 SharedPreferences，迁移到应用目录独立 JSON 存储
- [x] 本机备份 / 恢复纳入进度与“已听”状态，并兼容旧备份
- [x] 文档与 `AppBrand` / 产物名对齐为 `2.0.1`
- [x] 版本 `2.0.1+37`（Android `versionCode` 37）

### 2.0.0（2026-09-04）

- [x] 主线稳定版：合入 1.6.0 / 1.6.1 全部能力；不改播放器构造、不做前台服务开关、不迁进度库
- [x] 文档与 `AppBrand` / 产物名对齐为 `2.0.0`
- [x] 版本 `2.0.0+36`（Android `versionCode` 36）

### 1.6.1（2026-09-04）

- [x] Android 播客开播切 speech 会话：仅 `playItem` 停源之后、拉流之前，且种类变化时 configure；重试 / ICY / 暂停不切
- [x] Android 蓝牙连回续播：默认关；只听 `audio_session` 设备事件；不重复处理拔出；投屏/小睡不抢恢复
- [x] 紧凑列表：设置 → 外观，默认标准；只压电台/单集 `ListTile`（含未听 inbox、收听历史），不改全局 `ThemeData.visualDensity`
- [x] 正在播放指示：选中底 + leading 静态播放图标（台标角标 / 单集 `play_arrow`）；不用 `positionStream` 频谱
- [x] 首次探测可取消、可先听已测到的：探测中列表随测通增长；「停止检测」保留已测到的
- [x] 关键页面 widget 测试：`test/key_screens_test.dart`（探测进度、首次选台、备份、发现搜索、紧凑列表、电台空筛选）
- [x] 文档与 `AppBrand` / 产物名对齐为 `1.6.1`
- [x] 版本 `1.6.1+35`（Android `versionCode` 35）

### 1.6.0（2026-09-04）

- [x] 氛围包：废土终端 / 第三新东京 / 夜之城
- [x] 精选台标补 jpg/png；通知栏与列表共用 resolve，丢掉 favicon.ico
- [x] 播完自动下一集跳过已听
- [x] `PodcastFeedLogic.isDeniedCatalogFeed` 拦喜马拉雅 album / RSSHub / 荔枝
- [x] 发现播客：iTunes 搜索 + xyzrank 热榜；Podcast Index 高级；GetPodcast 外链
- [x] Android 直播缓冲只改构造默认值；列表滑动；迷你条滑切台/跳秒；章节跳转
- [x] 精选测活：删除 3 个蜻蜓 404 与汕头综合广播；央广 ytlive 换新签（约 409 台）
- [x] 本机整包备份 / 恢复（设置 → 数据管理；不含下载文件与 API 密钥）
- [x] Android Auto 播客：继续收听 + 已下载（上限 40）
- [x] Feed 缓存未听 inbox：播客页顶部；6 小时最多刷新 12 个订阅；打开节目也写入缓存
- [x] 播客页搜索可读缓存里的单集标题，不再为搜索现场拉全部 RSS
- [x] 按节目「自动下载最新一集」开关（默认关；遵守仅 WiFi；缓存刷新时下最新一集）
- [x] 单集时间戳书签：Now Playing / 单集菜单；本机笔记不分享；删订阅清掉
- [x] Windows 托盘：关主窗隐藏进托盘，音频继续；托盘可还原 / 播停 / 退出；图标失败时 × 仍退出
- [x] Windows 键盘快捷键：空格播停（输入或按钮焦点时不抢）；← / → 仅播客按跳秒档
- [x] Windows 开机启动 / 启动即迷你窗（默认关；设置 → 播放与收听；Debug exe 不写开机项；首次选台用完整窗口）
- [x] Android 小组件：续播未听完单集或上次收听；下一台切电台（冷启动去重；等电台列表加载）
- [x] 文档与 `AppBrand` / 产物名对齐为 `1.6.0`
- [x] 版本 `1.6.0+34`（Android `versionCode` 34）

### 1.5.4（2026-08-28）

- [x] 空订阅主按钮搜索节目；收听 tab 继续收听；电台收藏置顶
- [x] 长按标已听/未听、收藏单集；节目详情全部 / 未听 / 已下载 / 收藏
- [x] 播完自动下一集跳过已听（手动队列不跳）
...
- [x] Android 暂停后保留前台服务：`androidStopForegroundOnPause: false`；按 `audio_service` 约束同步关闭强制 ongoing 通知
- [x] 播客进度 / 已听状态迁出 SharedPreferences：新建 `PodcastEpisodeStateStore`，写入应用支持目录 `podcast_episode_state.json`
- [x] 兼容旧数据与旧备份：启动迁移 `podcast_progress_<guid>` / `listened_episode_guids`，恢复备份时仍能读旧 key；新备份使用 `podcastState`

### 仍可排后

- 功能候选见 [ROADMAP.md](ROADMAP.md)「下一轮（2.0.0 之后）」。源维护仍按 [SOURCES.md](SOURCES.md) 定期测活。央广 ytlive 探测 403 不要当死链删；签名过期用 `tools/refresh_radio_cn.py --apply`。

---

## 四、约定

1. 精选 JSON **宁缺毋滥**。稳定可播的国内台才写入 `assets/stations_cn.json`。添加前必须 GET 正文：HLS 要有 `#EXTM3U` 和至少一条非注释地址；JSON / HTML 即使 HTTP 200 也不入库。不要为了覆盖面堆台。台标写蜻蜓 `imgUrl` / 央广 `image` 的 jpg/png，不要写网站 `favicon.ico`；已有目录补图用 `tools/backfill_station_artwork.py --apply`。**首次启动**须先选收听范围（类型 / 省份 / 全部精选，至少一项），确认后才探测；探测中列表随测通增长，可「停止检测」保留已测到的。之后只在下拉刷新或设置「电台管理 → 检测可播放的源」时测全部源，连不上的不出现在主页。修改「收听范围」会重置探测。用户可隐藏不想听或听不了的台（本机名单，不改 JSON）。本机点播后一直缓冲或播失败也会按隐藏处理。探测不模拟播放。「刷新电台列表」只更新目录。
...
10. `RadioAudioHandler` 的 `AudioPlayer` 只构造一次，与 `AudioService.init` 同生共死。`AndroidLoadControl` / `AudioPipeline` 只能在构造时传入；禁止为换缓冲档或音效 `dispose` 再建（会打乱睡眠淡出、Cast、进度、Windows 换台）。不做音量增强 / LoudnessEnhancer。Android 音频会话只在电台↔播客种类变化、停源之后拉流之前 `configure`，不要在 ICY / position / 暂停时反复切。滑动手势仅 Android 列表与迷你条标题区；Windows 用右键；`DeskMiniBar` 不滑切台。
