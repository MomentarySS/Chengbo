...
class RadioAudioHandler extends BaseAudioHandler with SeekHandler {
  ...
  PlaybackSessionProfile _sessionProfile = PlaybackSessionLogic.startupProfile;
  ...
  bool bluetoothResumeEnabled = false;
  bool Function()? bluetoothResumeBlocked;
  ...
  Future<void> _startPlayback(PlaybackItem item, int request) async {
    ...
    try {
      ...
      if (!_shouldContinueStartup(request)) return;
      await _ensureSession(item.kind);
      if (!_shouldContinueStartup(request)) return;
      if (localPath != null) {
  ...
  void setPodcastChapters(List<PodcastChapter> chapters) {
    _chapters = chapters;
  }

  Future<void> skipPodcast(int delta) async {
    if (_currentItem?.kind != PlaybackKind.podcast) return;
    final target = PodcastChapterLogic.skipTarget(
      chapters: _chapters,
      position: _player.position,
      delta: delta,
    );
    ...
    await seekBy(delta > 0 ? skipStep : -skipStep);
  }

  @override
  Future<void> fastForward() => skipPodcast(1);

  @override
  Future<void> rewind() => skipPodcast(-1);
  ...
  Future<void> _ensureSession(PlaybackKind kind) async {
    if (!PlaybackSessionLogic.shouldReconfigure(
      offered: PlaybackSessionLogic.offeredOnThisPlatform,
      current: _sessionProfile,
      nextKind: kind,
    )) {
      return;
    }
    final next = PlaybackSessionLogic.profileFor(kind);
    if (await PlaybackSession.configure(next)) {
      _sessionProfile = next;
    }
  }

  Future<void> _attachBluetoothResume() async {
  ...
  void _onDevicesChanged(AudioDevicesChangedEvent event) {
    if (!BluetoothResumeLogic.shouldResume(
      enabled: bluetoothResumeEnabled,
      offered: true,
      userWantsPlayback: _userWantsPlayback,
      hasItem: _currentItem != null,
      alreadyPlaying: _player.playing,
      bluetoothOutputAdded: BluetoothResumeLogic.addedBluetoothOutput(
        added: [
          for (final device in event.devicesAdded)
            (isOutput: device.isOutput, typeName: device.type.name),
        ],
      ),
      blocked: bluetoothResumeBlocked?.call() ?? false,
    )) {
      return;
    }
    unawaited(play());
