...
abstract final class PodcastChapterLogic {
  ...
  /// 有章节时按章跳。没有下一章返回 `null`，调用方回退 ± 秒。
  static Duration? skipTarget({
    required List<PodcastChapter> chapters,
    required Duration position,
    required int delta,
    Duration restartWindow = chapterRestartWindow,
  }) {
    if (chapters.isEmpty || delta == 0) return null;
    final toc = tocOf(chapters);
    final list = toc.isEmpty ? chapters : toc;
    if (delta > 0) {
      for (final chapter in list) {
        if (chapter.start > position + const Duration(milliseconds: 400)) {
          return chapter.start;
        }
      }
      return null;
    }
    final current = atPosition(chapters: list, position: position);
    if (current == null) return Duration.zero;
    if (position - current.start > restartWindow) return current.start;
    PodcastChapter? previous;
    for (final chapter in list) {
    ...
    return previous?.start ?? Duration.zero;
  }
