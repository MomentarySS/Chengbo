...
class StationsNotifier extends StateNotifier<AsyncValue<List<RadioStation>>> {
  ...
  final Ref _ref;
  CancelToken? _probeCancel;

  /// 停止正在进行的批量探测，保留已经测到的台。
  void cancelProbe() {
    final token = _probeCancel;
    if (token == null || token.isCancelled) return;
    token.cancel('user-cancel');
  }

  void _publishProbeProgress({
    required int done,
    required int total,
    required int found,
  }) {
  ...
  Future<StationReloadResult> reload({bool forceProbe = false}) async {
    _probeCancel?.cancel('replaced');
    _probeCancel = null;
    final keepList = state.hasValue && (state.value?.isNotEmpty ?? false);
    if (!keepList) {
...
class StationSkipController {
  ...
  Future<void> skip(int delta) async {
    ...
    final queue = StationSkipLogic.queue(
      currentId: currentId,
      filtered: filteredAsync.value ?? [],
      favorites: _ref.read(favoriteStationsProvider).value ?? [],
      visible: visibleAsync.value ?? [],
    );
...
final autoBrowseSyncProvider = Provider<void>((ref) {
  ...
  void publish() {
    ...
    final feeds = ref.read(subscribedFeedsProvider).value ?? const [];
    final titles = {for (final feed in feeds) feed.id: feed.title};
  ...
  ref.listen(resumeListeningProvider, (_, __) => publish());
  ref.listen(podcastDownloadsProvider, (_, __) => publish());
  ref.listen(subscribedFeedsProvider, (_, __) => publish());
});
