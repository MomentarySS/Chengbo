...
- Android 13+ requests notification permission at launch. Current version `1.5.3+32`. Pack with `scripts/pack.ps1` to `dist/chengbo-1.5.3.apk`, `dist/chengbo-windows-1.5.3.zip`, and `dist/chengbo-windows-1.5.3.exe` (zip is the portable tree; exe is the Inno Setup installer; both ship `Chengbo.exe`. Release-signed when `android/key.properties` is present, minSdk 23). Windows builds need Visual Studio 2022 Build Tools with C++ ATL. Desktop HTTP follows the system proxy and common local Clash ports so public RSS hosts such as SoundOn can be fetched.
