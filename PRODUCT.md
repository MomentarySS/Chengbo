...
- Android 13+ requests notification permission at launch. Current version `2.0.1+37`. Pack with `scripts/pack.ps1` to `dist/chengbo-2.0.1.apk`, `dist/chengbo-windows-2.0.1.zip`, and `dist/chengbo-windows-2.0.1.exe` (zip is the portable tree; exe is the Inno Setup installer; both ship `Chengbo.exe`. Release-signed when `android/key.properties` is present, minSdk 23). Windows builds need Visual Studio 2022 Build Tools with C++ ATL. Desktop HTTP follows the system proxy and common local Clash ports so public RSS hosts such as SoundOn can be fetched.
...
- UI copy is Chinese.
- User-Agent: `Chengbo/2.0.1 (Flutter; chengbo radio)`.
- License: MIT.
