# CHANGELOG

All notable changes to **澄波 (Chengbo)** are documented here.

Format follows [Keep a Changelog](https://keepachangelog.com/zh-CN/), and the project follows [Semantic Versioning](https://semver.org/lang/zh-CN/).

---

## [Unreleased]

## [2.2.1] - 2026-09-24

### Added
- Windows 侧栏窗口：集中显示当前播放、手动队列和收藏电台，可与浮条及完整窗口互切；支持托盘、Ctrl+Shift+S、贴边吸附和位置恢复
- 播客订阅逐节目通知静音、最近成功刷新时间和手动立即刷新；后台通知仍服从全局开关
- 不可达电台支持逐台流地址检测，显示探测结果且不改写保存的地址
- 待听 Inbox 支持批量加入播放队列，并可将已下载单集优先加入
- 播客单集支持收藏，并统一在「收听 → 收藏」中查看

### Changed
- 底部迷你条、完整播放器、Windows 浮条和侧栏的播放 / 暂停图标加入轻微淡入缩放切换，并遵循系统「减少动态效果」设置
- 设置首页按内容、播放、数据分组；播客首页只预览 3 条未听单集，可进入独立列表查看全部，避免订阅节目被长列表挤到下方
- 电台分类筛选栏调整为与收藏页一致的标签导航样式
- 移除废土终端、第三新东京、夜之城三套氛围皮肤，保留澄波主题、浅深色和动态配色

## [2.2.0] - 2026-09-24

两页瘦身 + 电台页打磨。**本版合并了原 v2.1.1 的三处修复**（决定不单独发 v2.1.1）。
计划与逐条取舍见 [工单](../../docs/design/mobile-v2-2-density-work-order.md)，
效果图见 [docs/design/podcast-density-design.html](../../docs/design/podcast-density-design.html)。

### Added
- **发现播客搜索三级兜底**：iTunes → Podcast Index（填过免费密钥自动兜底）→ **本机目录**（GetPodcast 精选 + xyzrank 榜单前 1000，约 1250 个中文节目；拉一次存本机、搜索走本机）。结果上方标明来源与收录条目数；三级都不行时报**真实原因**并给出可照做的下一步
- **拉取失败回落本机缓存**：订阅的源暂时打不开时，详情页改用本机快照渲染（最多 40 集），并在筛选行下方说明「源暂时打不开，下面是本机缓存」+ 重试 —— 以前是整页报错，连已缓存的单集都够不着
- **详情页「节目设置」面板**：三个下载策略 + 跳过片头/尾收进一个面板（分「下载」「播放」两组），入口行只留一行状态摘要；面板里另有「仅WiFi下载」的**只读**状态行（开关本体在 设置 → 播放与收听）
- **`scripts/git-proxy.ps1`**：探测本机代理端口（Clash / NekoBox 等）再执行 git / gh，失败时区分「没人代理」与「代理通了但目标不可达」

### Changed
- **播客详情页 + 播放器页瘦身（8 条）**：下载开关收成一行入口（省 ~145px）、单集标题限 2 行、`仅WiFi下载` 移入设置、顶栏去掉重复的「选择多项」图标；播放器辅助行 5–6 个文字 chip 收成**一行图标**（保留 tooltip 与无障碍语义）、`跳过片头/尾` 移入详情页面板、去掉「停止」（迷你条 ✕ 即同一动作）
- **两页共享播放器视觉规格**：封面 / 台名卡的 `anchorMaxSide`、圆角与背景渐变统一到 `ChengboSkinTheme`（消除此前「电台 300 / 播客 360」的分叉）
- **睡眠倒计时收进顶部窄带**：两页共用一条**固定 24px** 的窄带（顶部栏与封面之间）。高度固定是为了避免开定时让封面重新分配空间、视觉上跳一下
- **订阅拦截收窄到只剩 RSSHub**：实测喜马拉雅 `album`、荔枝 `rss.lizhi.fm`、蜻蜓、小宇宙返回的都是**平台自己提供的标准 RSS 2.0**，与「第三方转接源」性质不同 —— 现在都允许订阅；拦截也不再作用于读取路径（已订阅的节目不会变成打不开的死链）。喜马拉雅裸专辑页会自动补成平台自己的 `.xml` 出口
- 电台页：生成式台名卡替代 Now Playing 封面、月亮图标在定时开启时点一下直接关闭、播放列表改为可上拖的半屏 sheet
- 设置 → 播放与收听新增「仅WiFi下载」（全局开关，原在单个节目页里，语义错位）

### Fixed
- **单集长按菜单尾部被裁（v2.1 引入）**：8–9 条 + 标题两行超过底部 sheet 的 9/16 高度上限，「复制地址」只露一半、「分享」完全够不到
- **睡眠定时图标点一下不关闭**：图标与 tooltip 都写着「关闭睡眠定时」，但 `onPressed` 只打开面板 —— tooltip 在说谎，且比电台页多一次点击
- **移动网络下「全部下载」没被拦住（v2.2 开发中引入）**：`downloadWifiOnlyProvider` 是懒创建的 `AsyncValue`，第一次读它是 `AsyncLoading`、`.value == null`，被当成「没开」；详情页那个常驻开关搬走后没人预热它。现在没加载完时直接问存储
- **「跳过片头/尾」面板显示不完整**：既没设 `isScrollControlled` 也没有滚动容器 → 底部「保存」被静默裁掉且滚不到
- **「跳过片头/尾」面板首帧崩溃**：`late int` 字段由异步加载赋值，首帧 build 读未初始化字段抛 `LateInitializationError`（会先闪错误块，且加载完成前点「保存」会把已设值写成 0）
- **订阅拦截把已订阅的节目变成死链**：`resolveUrl()` 每次拉取都拦，导致已订阅的喜马拉雅 / 荔枝节目打开即整页报错

---

## [2.1.1] - 2026-09-23

修复 v2.1.0 的两处问题。

### Fixed
- **版本号不一致（v2.1.0 引入）**：`lib/core/brand.dart` 的版本常量停留在 `2.0.2`，导致 v2.1.0 的「关于」页显示 `v2.0.2`、隐私说明页显示 `User-Agent Chengbo/2.0.2`、本机备份 JSON 记录 `appVersion: 2.0.2`，且所有网络请求（电台流 / RSS / 目录 / 下载）的 User-Agent 都是 `Chengbo/2.0.2`。现与 `pubspec.yaml` 对齐
- **电台迷你条底部的 3px 主色条**：纯装饰、不承载信息（电台直播流没有时长语义），且被卡片圆角裁切后更像一条残留的背景色带；澄波皮肤无边框时尤其明显。现移除
- **播客 Now Playing「已下载」chip 被抬高**：辅助行里只有它用 `Chip` + `VisualDensity.compact`（布局盒 40，兄弟 `ActionChip` 都是 48），而 `Wrap` 默认按顶对齐 → 它的标签中心比兄弟高 4 逻辑 px。现改为按中心对齐（保留它略小的状态样式）

### Changed
- 电台迷你条不再随播放 / 暂停跳变 3px 高度（移除的那条只在播放中出现）

---

## [2.1.0] - 2026-09-23

mobile player 升级与 Android widget 视觉重做（[计划](../../docs/design/mobile-v2-1-plan.md) v1.4，
[实施追踪](../../docs/design/v2-1-release-tracker.md)）。

### Added
- mini player 标题行右侧显示播客单集精确剩余时间（`剩余 mm:ss` / `剩余 h:mm:ss`）。
  睡眠倒计时激活 / 缓冲中 / 出错时整块隐藏
- Android 桌面 widget 跟随系统壁纸配色（Material You 动态色；尊重 App 内「壁纸 / 系统配色」
  开关，关闭时退回品牌深澄蓝）
- Android 桌面 widget 深色系统模式配色变体（之前只有浅色）
- Android 桌面 widget 采用 Material 3 矢量图标（替代旧系统 `ic_media_*` 拟物图标）
- 新增 Android「待听」桌面 widget（4×2 cell）：列出 inbox 前 4 个未听单集（标题 + 节目名），
  点击直接播放；空 inbox 显示「暂无未听单集 · 点此打开澄波」
- 关于页展示品牌口号（沿用 tagline 之后的扩展文案）

### Changed
- mini player 剩余时间数据源复用现有 `_MiniProgressBar` 的同一套 `positionStream`
  （两个 `StreamBuilder` 并发订阅安全）
- widget 颜色与「壁纸 / 系统配色」开关联动（关闭时退回品牌深澄蓝；不再硬编码深澄蓝）
- widget 根背景 / 文字色受深色系统模式影响（之前只有浅色变体；新增 `values-night/colors.xml`）

### Fixed
- 清理 `ArtworkUrlLogic.resolve` 中的死分支（jpg/jpeg/png/webp + 3 个 CDN host 的 if 块与
  fall-through 等价）—— 行为不变
- 修正 ROADMAP 中「电台累计」条目的错误语义（电台直播流没有「总时长」概念）

### Known issues
- widget 续播按钮宽度 40dp 低于 Android 无障碍建议的 48dp（既有问题，本版不修）
- widget 配色跟系统深色模式，不跟 App 内「外观」开关（已知不一致，本版不修）

---

## 2.0.2 / 2026-09-22

播放性能与 I/O 优化，无新增功能，未改变对外行为。

- **播客进度落盘**：不再按秒 fsync 整个状态文件——周期性写盘放宽到 5 秒且只交给系统页缓存，暂停 / 停止 / 播完 / 退出时仍强制同步落盘。代价是进程被强杀时最多丢 5 秒进度。
- **下载进度节流**：进度变化 ≥1% 或间隔 ≥300ms 才更新界面；播客详情页只订阅已下载记录，单集列表不再跟着每个数据块重新排序和过滤。
- **首次探测**：已找到数量改为增量计数，去掉每台一次的全量可见列表快照与 `urlOk` 全量复制。
- **Windows 系统代理**：改用异步 `reg` 读取并在首帧前预热，不再在首个网络请求里同步起进程。
- **精选电台 JSON**：只解析一次；失败不缓存，之后仍可重试。
- **单集进度**：改为同步读取（数据本就在内存），`podcastProgressProvider` 加 `autoDispose`，逐集实例在行滚出屏幕后释放。
- **启动**：主页各 tab 惰性构建，不再在启动时一次性构建四个页面。
- **播客订阅**：`PodcastFeed` 按 id 判等，同一个节目不再重复拉取 RSS。
- 修复：音频会话配置移回首帧前（`RadioAudioHandler` 启动即 music，否则首次播放电台不会 configure）。
- User-Agent `Chengbo/2.0.2`
- 产物 `chengbo-2.0.2.apk`、`chengbo-windows-2.0.2.zip`、`chengbo-windows-2.0.2.exe`（Android `versionCode` 38）

---

## 2.0.1 / 2026-09-17

在 2.0.0 基础上的稳定性与数据兼容更新。

- 暂停播放后继续保留 Android 前台服务，通知可由用户移除。
- 播客进度与“已听”状态迁移到应用目录的独立 JSON 文件，并自动兼容迁移旧数据。
- 本机备份与恢复纳入播客进度和“已听”状态，旧版本备份仍可恢复。
- User-Agent `Chengbo/2.0.1`
- 产物 `chengbo-2.0.1.apk`、`chengbo-windows-2.0.1.zip`、`chengbo-windows-2.0.1.exe`（Android `versionCode` 37）

---

## 2.0.0 / 2026-09-04

主线稳定版。相对上次发布的 **1.5.4**，合入原计划 1.6.0 / 1.6.1 的全部能力（发现播客、本机备份、Windows 桌面、speech 会话、蓝牙连回、紧凑列表、正在播放指示、首次探测可取消等）。细项见下方 1.6.0、1.6.1。暂停后前台服务与进度/已听迁库仍按路线图排后，本版不做。

- User-Agent `Chengbo/2.0.0`
- 产物 `chengbo-2.0.0.apk`、`chengbo-windows-2.0.0.zip`、`chengbo-windows-2.0.0.exe`（Android `versionCode` 36）

---

## 1.6.1 / 2026-09-04

- **播客 speech 会话**：Android 在电台↔播客换种类、拉流之前才切 `AudioSession`（电台 music / 播客 speech）。同类换台、下一集、暂停、ICY 不再 configure。Windows 仍用启动时的 music。
- **蓝牙连回续播**：设置 → 播放与收听，仅 Android，默认关。打开后蓝牙耳机再次连上且用户仍想播时继续播放；拔出暂停仍走系统，不与投屏或睡眠小睡抢恢复。
- **紧凑列表**：设置 → 外观，默认关。只给电台行、单集行、未听 inbox、收听历史设 `ListTile` compact；不改全局 `ThemeData.visualDensity`，底栏和芯片保持标准密度。
- **正在播放指示**：当前行仍用选中底；电台/封面右下角叠静态播放图标，单集 leading 改为 `play_arrow`。不用 `positionStream` 频谱，收听历史也不再用 equalizer。
- **首次探测可先听 / 可取消**：检测直播源时不再整页卡住。已测通的台马上出现在列表里；进度条可「停止检测」，已测到的会留下来。
- **关键页面 widget 测试**：`test/key_screens_test.dart` 覆盖探测取消文案、首次选台、本机备份、免密钥搜索、紧凑列表默认关、电台筛选空态。

---

## 1.6.0 / 2026-09-04

- **氛围包**：设置 → 外观可选废土终端、第三新东京、夜之城；锁深色，换种子色、叠纹、迷你条光晕与探测/空状态短句。列表与迷你条结构不变。切回澄波后恢复浅深色与壁纸配色。
- **精选电台台标**：`stations_cn.json` 补入蜻蜓 `imgUrl` 与央广 `image` 的 jpg/png；不再用官网 `favicon.ico`。通知栏 / 锁屏封面与列表共用同一套 resolve。
- **播完跳过已听**：自动下一集仍按当前排序往下走，但会跳过已听完的单集。手动播放队列不跳过。
- **发现播客**：同一全屏页分段「搜索 | 中文热榜」。搜索默认 iTunes（免密钥）；xyzrank 热榜分页；Podcast Index 折进高级。空订阅、设置「播客管理」、有订阅时「发现节目」进同一页。
- **版权库拦截**：喜马拉雅 `album`、RSSHub、荔枝转接无法订阅；手动粘贴、OPML、发现页共用。
- **听感护栏**：Android 直播缓冲只改构造默认值；电台左滑隐藏 / 右滑收藏、单集左滑已听 / 右滑下载（仅 Android）；手机迷你条左右滑切台或跳秒；有章节时控制键与耳机键按章跳。
- **精选测活**：去掉 3 个蜻蜓 HTTP 404 和超时的汕头综合广播（约 409 台）。央广 ytlive 的 `key`/`time` 约两周失效，已原位换新签（保留电台 id）；探测 403 不要当死链整表删。
- **本机备份**：设置 → 数据管理可导出 JSON（收藏、订阅、进度、隐藏台、收听范围等），从剪贴板恢复；不含直播、已下载音频和 Podcast Index 密钥。恢复后需完全退出再打开。
- **车机播客**：Android Auto 浏览树增加「继续收听」和「已下载」（各最多 40）；点选走现有播放器，已下载优先播本机文件。
- **未听 inbox**：播客页顶部列出各订阅最新一集未听（最多 20）。来自本机 Feed 缓存，不打开页面就拉全部 RSS；打开应用后每 6 小时最多刷新 12 个节目，打开节目详情也会写入缓存。订阅搜索同时匹配缓存里的单集标题。
- **自动下载最新一集**：节目详情可按订阅打开开关（默认关）。打开后刷新该节目或 Feed 缓存更新时只下最新一集；遵守「仅 WiFi 下载」；删除订阅会关掉。
- **时间戳书签**：播客 Now Playing「书签」和单集长按菜单可在当前秒加一条本机笔记（可留空）；点按跳到该进度；不分享。同一秒再加会覆盖笔记。删除订阅会清掉该节目书签。
- **Windows 托盘**：点标题栏 × 缩小到托盘，音频继续。托盘左键还原窗口，右键可播停或退出。迷你窗的 × 仍是回到完整窗口。图标或菜单没挂上时 × 仍是退出，避免窗口藏进任务栏外。
- **Windows 快捷键**：空格播停（输入或按钮焦点时不抢）；← / → 仅播客按当前「快进 / 快退秒数」跳转（有章节时与屏幕按钮一样按章跳）。
- **开机启动 / 启动即迷你窗**：设置 → 播放与收听，默认关。开机启动写本机用户注册表，登录后打开澄波但不自动播放；`flutter run` 的 Debug 构建不写入。启动即迷你窗每次打开都先显示迷你窗；首次选收听范围时仍用完整窗口。
- **Android 小组件**：除播放/暂停外可点「续」继续收听未听完单集（没有则播上次电台/单集），点下一台切当前列表的下一座电台。冷启动只处理一次点击；电台列表未加载完时会等一会儿再切。

---

## 1.5.4 / 2026-08-28

- **界面露出**：空订阅主按钮改为搜索节目；收听 tab「最近」也有继续收听；电台列表收藏置顶。
- **单集筛选**：长按可标已听/未听、收藏单集；节目详情芯片为全部 / 未听 / 已下载 / 收藏。
- **播放控制**：± 秒数可选 10/15/30/60（长按改档）；睡眠定时记住上次；可再听 2/3 集。
- **单集章节**：Feed 内读 Podlove；有 JSON 地址时播放或点开再拉；封面下显示当前章名，进度条打点。

---

## 1.5.3 / 2026-08-28

- **下载进度不被清理冲掉**：自动清理后刷新下载列表只更新已落盘记录，进行中的进度、失败状态和失败提示序号会保留。
- **删订阅停掉进行中的下载**：删除节目时取消该 feed 正在下载的任务，避免文件写回；内存里的进度和失败标记一并清掉。
- **测试**：并行队列、勾选待下、失败 SnackBar 判定、按节目删除、reload 保进度。

---

## 1.5.2 / 2026-08-28

- **最近 N 集**：不再依赖「全部下载」开关；按最新在前取未下载的前 3/5/10 集，与列表排序无关；点击后有开始或已全部下载的提示。
- **单集下载菜单**：长按或 Windows 右键打开同一份菜单；下载中可取消，失败可重新下载，已下载可删除。
- **行内进度**：节目详情单集行显示「正在下载 xx%」或「下载失败」，下载中在该行显示进度条。设置「数据管理」仍只显示占用与清除。
- **勾选多集**：顶栏勾选图标与菜单「选择多项」进入选择模式，可全选当前列表并下载所选。
- **下载入口补强**：返回键先退出选择模式；排序/刷新/删除收进「更多」；全部下载与最近几集按完整节目列表计算；Now Playing 可下载 / 取消 / 重试当前集。
- **电台右键**：电台列表 Windows 右键与长按打开同一份菜单。
- **下载清单**：设置 → 数据管理 → 播客下载可点进已下载列表，可删单集或全部清除。
- **批量并行**：全部 / 最近 N / 勾选下载最多同时 2 路。
- **失败提示**：下载失败弹出 SnackBar，不发系统通知。

---

## 1.5.1 / 2026-08-27

- **已听完重播**：开播前的 seek 不再把已听完进度当成续播位置，避免立刻 `completed` 跳到下一集；跳过片头仍生效。
- **下载自动清理**：启动时等已听名单就绪再清理；缺完成时间的旧下载视为已到期；打开开关或改天数会立刻跑一轮；清理后刷新下载列表。
- **按节目倍速**：未单独设过的节目回退到上次全局倍速；速度面板改完会立刻更新选中态。
- **手动播放队列**：先播放成功再出队，失败不会丢掉这一集；队列可查看、删除和排序全部条目。
- **投屏开关**：Chromecast 入口只在 Android 外观页出现。
- **文档**：Now Playing 顶栏、迷你窗平台、仅 WiFi 下载入口、inbox 状态、隐私文案与版本号与代码对齐。

---

## 1.5.0 / 2026-08-27

- **电台搜索增强**：搜索栏新增码率过滤 chip（全部 / 64k+ / 128k+ / 256k+），一键筛选高品质 streams；新增收藏筛选 toggle，可只看已收藏电台；搜索历史记忆最近 5 条查询，focus 时下拉显示，支持一键清除。
- **睡眠定时器增强**：预设扩展为 5 / 10 / 15 / 20 / 25 / 30 / 45 / 60 分钟；新增 30 秒淡出关闭（定时结束前 30 秒音量渐降，比突然断声更舒适）；新增「小睡 10 分钟」按钮（立刻暂停，到点后自动续播）；倒计时最后 30 秒显示「淡出中」提示。
- **跳过片头/片尾**：按播客 feed 分别记忆片头和片尾跳过秒数，10 档可选（0 / 5 / 10 / 15 / 20 / 30 / 45 / 60 / 90 / 120 秒）；播放时自动 seek 到片尾之后，后退按钮不会退过片头起点，接近片尾时自动跳至末尾。入口在 Now Playing 页的「跳过片头/尾」chip。
- **播客下载管理增强**：已下载单集 subtitle 显示文件大小（如「已下载 (12.3 MB)」）；全部下载行显示该 feed 已下载总量；新增「仅 WiFi 下载」全局开关（纯移动网络跳过，Wi-Fi / 有线允许）；新增「下载最近 N 集」下拉菜单（3 / 5 / 10 集一键下载）。
- **收听历史导出**：收听 tab「最近」页播客区右上角新增「导出」按钮，一键导出 JSON（含收听历史 + 收听统计），通过系统分享发送。
- **Show Notes 查看器**：播客单集列表 tile 右侧新增备注图标按钮，点开直接浏览该集简介/Show Notes，无需先播放。
- **播放速度精细化**：倍速选择新增 0.5x 和 0.6x 慢速档，满足语言学习等慢听需求。
- **修复**：睡眠淡出不再把音量 0 写成上次音量；仅 WiFi 下载按连通性判断（纯移动网络跳过）；小睡到点后自动续播。

---

## 1.4.8 / 2026-08-19

- **移除收听统计页热力图**：近 26 周 GitHub 风格热力图整体移除；统计页保留累计收听、今日/本周、电台 vs 播客占比条、最常收听 Top 5，表达收听时长信息不丢失，界面更清爽。

---

## 1.4.7 / 2026-08-19

- **内置电台列表重建为 413 台**：`assets/stations_cn.json` 从 265 台重建为 **413 台**（蜻蜓省市级 + 央广官方 19 + 广东本地补充 东莞等）。来源从 4 个网站抓取：蜻蜓 FM `radiopage` 各省级频道（GraphQL `radioPage(cid, page){contents}`，流地址统一为 `https://lhttp.qtfm.cn/live/{id}/64k.mp3`，31 省全覆盖）、央广网官方 `appBroadcast/list`（签名 API，含中国之声/经济之声/大湾区之声等 19 套央广频率）。每台经两轮全量探测 + 最终存储 URL 复验（含 gzip 解压、重定向跟随、普通 GET 优先），按 URL 与规范化台名去重，只保留两轮全过且存储链接可播的台。
- **电台探测逻辑修正**：`StreamUrlTester` 与 `tools/stream_content.py` 原为 Range 请求优先，蜻蜓等 CDN 对 Range 回 404 导致误杀；现改为普通 GET 优先（等价真实播放），Range 仅兜底，并支持 gzip 解压（新城电台等源的 m3u8 带 gzip 压缩）。探测语义同步更新。
- **新增电台目录维护工具**：`tools/rebuild_curated_stations.py` 从蜻蜓 / 央广 / tingfm / radio5 四源收集、探测、去重并重建精选 JSON，带断点缓存（`tools/_scrape_cache/`）；`tools/prune_dead_stations.py` 按需清理死链。
- 遗留：tingfm（港澳台/网络台）与 radio5 因目标站限流暂未并入，脚本可在限流解除后补跑合并。

对应版本：`1.4.7+27`（Android `versionCode` 27）。

- **广东电台列表补充东莞**：运行 `tools/merge_gd_stations.py` 恢复东莞阳光 1008 与东莞畅享 1075 两台，流地址经 HLS 播放列表验证可用。广东精选列表从 56 台恢复为 58 台，21 个地级市全覆盖。

---

## 1.4.6 / 2026-08-18

- **Windows 电台卡住修复**：`just_audio_windows` 的事件回调原来把 `PlaybackStateChanged` 等事件 Post 到 UI 线程再发 `EventSink`，导致 Dart 端永远收不到播放状态更新——MF 其实已在播放但 UI 一直停在「正在缓冲」，18 秒后看门狗重试陷入循环。修复为与官方实现一致：直接在 Media Foundation 回调线程发送事件，保留 `mutex_` 与 `disposed_` 线程安全保护。另移除播放前对 `mediaPlayer.Play()` 的重复调用与 load 末尾的 `broadcastState()`（会用过期的 None/Opening 状态覆盖 Playing 事件），恢复 `loadSource` 开头的 `Pause()` + `Source(nullptr)` 换源清理。
- **「未听」分页移除**：收听 tab 的「未听」分页因跨订阅逐个拉取 RSS 导致加载很慢，直接删除；inbox 相关 provider 与逻辑（`inboxProvider`、`PodcastInboxLogic`、`EpisodeWithFeed`）一并清理。收听 tab 恢复为 收藏 / 最近 / 统计 三段。
- **长按菜单统一替换列表常驻按钮**：电台列表、播客订阅列表、播客单集列表均移除右侧常驻 hide/favorite / 下载 / 简介按钮，改为长按弹出底部菜单，菜单包含播放、收藏/取消收藏、复制地址、分享、删除，列表更清爽。
- **电台长按菜单补回收藏**：用户反馈去掉收藏入口后不便使用，现重新加入长按菜单，并根据当前是否已收藏动态显示「收藏」或「取消收藏」。
- **播客订阅列表长按菜单**：新增长按底部菜单，支持播放最新、复制 RSS 地址、分享、删除订阅；列表项 trailing 删除按钮同步移除。
- **播客单集长按菜单**：长按单集弹出底部菜单，支持查看简介、加入播放队列、下载到本机 / 删除下载、复制音频地址、分享；列表项 trailing 下载 / 简介按钮同步移除。
- **设置页「源」改名为「电台管理」**：入口名称与图标同步更新，更清晰表达该页负责电台相关内容。
- **收听 tab 曾短暂增加「未听」分页后移除**：播客主页的「未听单集 inbox」先移到「收听」tab，因加载慢最终整体删除。
- **Now Playing 顶部栏标题精简**：电台与播客两款播放器顶部去掉「电台」「播客」标题文字，保留返回按钮与投屏按钮。

对应版本：`1.4.6+26`（Android `versionCode` 26）。

---

## 1.2.13 / 2026-08-18

- **播客进度跨单集污染修复**：`RadioAudioHandler._currentItem` 原来在 `playItem` 开头就换成新单集，早于 `_safeStop()` 执行，切换窗口内旧单集的实时位置会被写进新单集的进度 key，导致新一期"不是从最开始播放"。修复后 `_currentItem` 赋值延迟到 `_safeStop()` 之后，且 `_emitPlayError` 改为显式传入目标单集，避免错误报告对象。
- **已听完单集重播修复**：播完后的进度等于完整时长，重播时 seek 到结尾会立即触发 completed → 自动跳到下一集，导致无法从头重听。seek 前用 `PodcastPlaybackLogic.isFinished` 判断，已听完的单集从头开始。
- **自动推进只在真正播完时触发**：原来 `onPositionTick` 在播到 95%（或剩余 ≤15 秒）就提前标记已听并推进队列下一集，长节目尾段被截断，且与 feed 自动下一集竞争。现在提前推进逻辑全部移除，只在 `processingState == completed` 时统一处理：标记已听 → 检查睡眠定时 → 手动队列优先 → feed 自动下一集。
- **播客单集自然排序**：国内 RSS 常缺发布日期，标题 fallback 改用自然序比较（数字段按数值比），避免 `#1 → #10 → #100 → #2` 的字典序错乱，且排序方向跟随「最新在前 / 最早在前」模式。
- **Now Playing 顶部栏统一**：两款播放器去掉顶部的「电台」「播客」标题文字，间距统一为 8，节目名和播客名/台名改为居中对齐。
- **进度写盘节流**：播客进度从每 200ms 写一次 SharedPreferences 改为最多每秒一次，暂停、停止、播完时强制落盘，减少平台通道调用次数。
- **版本号同步修复**：`lib/core/brand.dart` 版本号与 `pubspec.yaml` 不同步的问题修正。

对应版本：`1.2.13+25`（Android `versionCode` 25）。

---

## 1.2.12 / 2026-08-18

- **搜索播客移到播客管理页**：设置里「搜索播客（Podcast Index）」入口从「关于」页移到「播客管理」页，与添加 RSS、导入/导出 OPML 并列；关于页移除该入口。
- **设置页重新排序**：新顺序为 播客管理 → 电台管理 → 播放与收听 → 外观 → 数据管理 → 电台分类 → 关于；关于副文改为「版本、隐私说明」。
- **热力图空数据修复**：`_HeatCell._computeLevel` 在 `maxValue == 0` 时从返回 `1`（浅色模式下近透明绿）改为返回 `0`（空单元格灰），避免无数据时全部格子看起来失效。
- **收听统计持久化修复**：`ListeningStatsLogic.compact` 日期 key 解析从 `DateTime.tryParse(key.replaceAll('-', ''))` 改为 `DateTime.tryParse(key)`，因为 `dayKey` 生成标准 `YYYY-MM-DD`，去掉横杠后 Dart 不认，导致历史天数被静默丢掉。
- **Now Playing 标题居中修复**：`NowPlayingTopBar` 左侧 `IconButton` 加 `SizedBox(40×40)` 约束，消除隐形触控区，使「电台」「播客」标题视觉居中。
- **桌面迷你窗点击穿透修复**：`DeskMiniBar` 两个 `GestureDetector`（主条区域 + 圆形封面）加 `onTapDown` 消费点击，避免无边框窗口下首次点击穿透到下层 Scaffold 触发导航。

对应版本：`1.2.12+24`（Android `versionCode` 24）。


## 1.2.10 / 2026-08-17

- **播客管理进入设置页**：设置新增「播客管理」入口，集中放置「添加 RSS 订阅」「从剪贴板导入 OPML」「导出 OPML 到剪贴板」三项操作；入口副文为「RSS 订阅、OPML 导入导出」。
- **订阅数量实时展示**：播客管理页顶部监听 `subscribedFeedsProvider`，加载态、空态、出错态均有占位，数据就绪后显示「共 N 个订阅」。
- **播客页去除三项操作**：订阅列表内的「添加 RSS」「导入 OPML」「导出 OPML」三个 `ListTile` 移除，仅在设置「播客管理」操作；不再被调用的 `_exportOpml` 静态方法一并清除。
- **空状态引导保留**：没有订阅时的空状态仍保留「添加 RSS」和「导入 OPML」两个按钮，作为首次使用的引导入口。

对应版本：`1.2.10+22`（Android `versionCode` 22）。

---

## 1.2.9 / 2026-08-17

- （见更早 CHANGELOG 记录）

---

## 1.2.8 / 2026-08-17

- 播客主页增加搜索框，按订阅标题搜索，搜索时隐藏继续收听卡片和 inbox。

---

更早版本记录已归档。
