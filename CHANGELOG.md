...
- **睡眠定时器增强**：预设扩展为 5 / 10 / 15 / 20 / 25 / 30 / 45 / 60 分钟；新增 30 秒淡出关闭（定时结束前 30 秒音量渐降，比突然断声更舒适）；新增「小睡 10 分钟」按钮（暂停播放 + 重新计时）；倒计时最后 30 秒显示「淡出中」提示。
...
- **播客下载管理增强**：已下载单集 subtitle 显示文件大小（如「已下载 (12.3 MB)」）；全部下载行显示该 feed 已下载总量；新增「仅 WiFi 下载」全局开关（移动网络自动跳过下载）；新增「下载最近 N 集」下拉菜单（3 / 5 / 10 集一键下载）。
