...
- [ ] 播客播放中 → mini player 标题右侧显示 `剩余 mm:ss`
- [ ] 剩余时间随播放递减（约每秒刷新）
- [ ] 剩余时间超过 1 小时 → 显示 `剩余 h:mm:ss`
- [ ] 播客暂停 → 剩余时间保持显示且不变
- [ ] 播客缓冲中 → 剩余时间隐藏，副文显示「正在缓冲…」
- [ ] 播客播放出错 → 剩余时间隐藏，副文显示错误
- [ ] 电台播放中 → **不显示**剩余时间（保持 ICY 曲名 + 3px 主色条）
- [ ] 睡眠定时激活 → 剩余时间隐藏，只显示睡眠倒计时
- [ ] 睡眠定时取消 → 剩余时间恢复
- [ ] 超长单集标题 → 标题 ellipsis 正常，时间标签不被挤掉
- [ ] widget 动态色开关开启（API 31+）→ widget 背景跟随壁纸配色
- [ ] widget 动态色开关关闭 → widget 背景为品牌深澄蓝
- [ ] 系统深色模式 → widget 使用深色变体配色
