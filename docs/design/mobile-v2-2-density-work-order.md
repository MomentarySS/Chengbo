...
### 11.7 拉取失败回落到本机缓存（commit `4c11b8c`）

**起因**：源暂时不可达时，详情页整页变成「RSS 解析失败」+ 重试 —— 而本机缓存里通常还有最多 40 条单集、音频地址大多还能播（实测你缓存里 683 条音频地址大部分返回 206）。用户既看不到列表，也听不到已缓存的集。

**做法**：`podcastDetailProvider` 在 `fetchFeed` 失败时回落到 `feedCacheProvider` 里的快照：

- **回落前先从存储重读一次缓存**（`feedCacheProvider.notifier.reload()`）—— 冷启动时该 notifier 可能还没加载完，直接读 state 会拿到空 map，回落就静默失效了。
- **只在真的有缓存时回落**；没有缓存仍然把错误抛上去（第二条测试钉住这一点）。
- 回落后把 `detailFromCacheProvider(feedId)` 置为 true，页面在筛选行下方显示一条 `源暂时打不开，下面是本机缓存` + `重试` —— 否则用户会以为这是刚拉的列表。
