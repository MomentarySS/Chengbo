...
```powershell
curl.exe -sL --compressed --max-time 12 -A "Chengbo/1.5.0 (Flutter; chengbo radio)" "流地址"
```
...
核对：2026-08-18。User-Agent `Chengbo/1.5.0` 拉取 Feed，要求 HTTP 200、至少 1 条 `<enclosure>` 音频。来源是 iTunes `feedUrl`、节目官网，以及创作者自己打开的 `feed.xyzfm.fm`。Windows 拉境外 Feed 走系统代理，并探测本机 Clash 常见端口。
