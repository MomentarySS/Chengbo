...
* Post Media Foundation callbacks to the Flutter UI thread before touching EventSink (fixes non-platform-thread channel crashes).
* Pause and clear the current source before load; enable RealTimePlayback for HLS/DASH.
* Map Media Foundation Opening to buffering so live HLS can receive Play() instead of waiting forever.
