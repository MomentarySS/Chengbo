...
private:
  ...
  /// Dart may send play() before load() finishes; replay after the source is set.
...
public:
  ...
  void StartListening() {
    ...
    playback_state_token_ = mediaPlayer.PlaybackSession().PlaybackStateChanged(
        [weak](auto, const auto&) -> void {
          // Must touch the EventSink directly on the Media Foundation
          // callback thread (like upstream). Posting to the UI thread made
          // Dart never receive playback state updates.
    ...
    media_failed_token_ = mediaPlayer.MediaFailed(
        [weak](auto, const Playback::MediaPlayerFailedEventArgs& args) -> void {
          ...
          std::cerr << "[just_audio_windows] Media error: " << errorMessage
                    << std::endl;
    ...
    item_failed_token_ = mediaPlaybackList.ItemFailed(
        [weak](auto, const Playback::MediaPlaybackItemFailedEventArgs& args) -> void {
  ...
  void AudioPlayer::HandleMethodCall(
    ...
    if (method_call.method_name().compare("load") == 0) {
      ...
      try {
        ...
        return result->Error("load_error", "load failed");
      }
      ...
      // Dart may call play() before load() finishes. Re-play after source is set.
      if (play_when_ready_ && !disposed_) {
        try {
          mediaPlayer.Play();
        } catch (winrt::hresult_error const& ex) {
          std::cerr << "[just_audio_windows] Play after load error: " << winrt::to_string(ex.message()) << std::endl;
        }
      }

      result->Success(flutter::EncodableMap());
    } else if (method_call.method_name().compare("play") == 0) {
      play_when_ready_ = true;
      try {
        // Official upstream plays unconditionally; Media Foundation starts
        // playback automatically when a source is set while a play request is
        // pending. Playing with no source is safe and matches upstream.
        if (!disposed_) {
          mediaPlayer.Play();
        }
      } catch (winrt::hresult_error const& ex) {
        std::cerr << "[just_audio_windows] Play error: " << winrt::to_string(ex.message()) << std::endl;
      }
      result->Success(flutter::EncodableMap());
      ...
      try {
        ...
        std::cerr << "[just_audio_windows] Pause error: " << winrt::to_string(ex.message()) << std::endl;
      }
      result->Success(flutter::EncodableMap());
  ...
  void AudioPlayer::loadSource(const flutter::EncodableMap& source) {
    ...
    const bool live = type != nullptr &&
        (type->compare("hls") == 0 || type->compare("dash") == 0);
    live_stream_ = live;
    try {
      mediaPlayer.RealTimePlayback(live);
    } catch (...) {}

    if (type != nullptr && type->compare("concatenating") == 0) {
  ...
  */
  ...
  void AudioPlayer::broadcastState() {
    ...
    try {
      ...
      std::cerr << "[just_audio_windows] Broadcast data error" << std::endl;
    }
  }
  ...
  int AudioPlayer::processingState(Playback::MediaPlaybackState state) {
    ...
    if (state == Playback::MediaPlaybackState::None) {
      return 0; //idle
               ...
               state == Playback::MediaPlaybackState::Buffering) {
      return 2;//buffering
    } else if (state == Playback::MediaPlaybackState::Playing ||
               state == Playback::MediaPlaybackState::Paused) {
      return 3; //ready
    }
    if (!live_stream_) {
      auto duration = session.NaturalDuration().count();
      if (duration > 0 && session.Position().count() == duration) {
        return 4; //completed
      }
    }
    return 3; //ready
