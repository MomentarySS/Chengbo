...
产物在 `dist/`，文件名跟 `pubspec.yaml` 的 `x.y.z` 走（当前为 `chengbo-2.0.1.apk`、`chengbo-windows-2.0.1.zip`、`chengbo-windows-2.0.1.exe`）。改版本后需重新 `.\scripts\pack.ps1`。本机有 `android/key.properties` 时 APK 用正式密钥签名；没有则回退 debug 签名。`minSdk` 23。Windows 双击 `.exe` 安装包安装，安装后从开始菜单启动；安装包支持卸载（控制面板 / 设置 → 应用）。
...
- Radio Browser 会动态解析镜像；User-Agent 为 `Chengbo/2.0.1 (Flutter; chengbo radio)`
