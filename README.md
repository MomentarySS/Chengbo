...
当前版本：`1.4.8+28`（2026-08-19）。相对 1.4.7：移除收听统计页热力图，保留累计时长、电台/播客占比、最常收听 Top 5；统计页更清爽。
...
- 收藏、最近播放与收听统计统一在底部「收听」tab：收藏段保留收藏电台；最近段合并播客收听历史与电台最近播放，各自可清除；统计段展示今日 / 本周 / 总时长、电台 vs 播客占比、最常收听，本机记录可清除
...
- 播客 RSS 播客：自行添加订阅、删除、下拉刷新、单集简介、播放进度、按需下载离线听；Now Playing 可 ±15 秒与倍速（0.8–2×）。播完按当前排序自动下一集。可用剪贴板导入/导出 OPML。不自带默认播客。「最近」段记录最近 30 集，显示听到哪里，可点继续收听或清除
- 后台播放（Android 通知栏控制；13+ 首次启动会请求通知权限）
- Spotify 式播放器：迷你条点开 Now Playing 时封面有 Hero 动画；**电台与播客为同一套极简布局**（跟随系统深浅色，顶部有播放内容取色的柔和渐变）：顶部返回+「电台/播客」标题+投屏、大圆角封面（无图时占位图标）、左对齐标题、细调节条（播客为进度、电台为音量）、加大底部控制行——播客：倍速 / ±15 秒 / 大播放键 / 播放列表；电台：睡眠定时 / 上一台 / 下一台 / 播放列表；简介、已下载、睡眠定时、停止在标题下；睡眠定时（15/30/45/60 分钟、自定义，或播客「本集结束」）；右侧播放列表可切换电台或单集；直播流若带 ICY 元数据，迷你条与 Now Playing 会显示正在播放曲名，过长则跑马灯
...
- 设置：外观（跟随系统 / 浅色 / 深色）、壁纸/系统配色、记住上次收听、桌面迷你窗、摇一摇延长睡眠（仅 Android）、新一集通知（默认关，最少 6 小时，首次只记进度）、清除封面缓存、清除播客下载、Podcast Index 搜索（只在设置里提供）、播客管理（RSS 订阅、OPML 导入导出、订阅数量）、电台管理（收听范围、检测可播放的源、刷新电台列表、连不上的电台、Radio Browser、境外、手动添加）、[隐私说明](PRIVACY.md)
...
产物在 `dist/`，文件名跟 `pubspec.yaml` 的 `x.y.z` 走（当前为 `chengbo-1.4.6.apk`、`chengbo-windows-1.4.6.zip`、`chengbo-windows-1.4.6.exe`）。改版本后需重新 `.\scripts\pack.ps1`。本机有 `android/key.properties` 时 APK 用正式密钥签名；没有则回退 debug 签名。`minSdk` 23。Windows 解压后运行 `Chengbo.exe`，或双击 `.exe` 安装包解压到任意文件夹后运行 `launch.bat`。
...
├── app.dart
├── core/
│   ├── audio/          # 播放器、ICY、睡眠、摇一摇、Cast 逻辑、小组件快照
│   ├── category/       # 分类规则
│   ├── models/
...
└── shared/widgets/     # MiniPlayer、跑马灯、Cast、摇一摇监听
...
- Radio Browser 会动态解析镜像；User-Agent 为 `Chengbo/1.4.6 (Flutter; chengbo radio)`
