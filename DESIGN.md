---
name: 澄波
description: 听国内广播 — Material 3 列表与 Spotify 式迷你播放条（对应产品 2.1.0）
colors:
  chengbo-blue: "#1565C0"
  deep-chengbo: "#0D4F8C"
  primary: "#405F90"
...
澄波把整座应用铺成一盆静水。电台、播客、收藏、设置都在同一层冷蓝表面上滑动，没有卡片堆、没有装饰插画，也没有额外的氛围皮肤。主题支持浅色、深色、跟随系统，以及壁纸 / 系统配色。水纹只出现在正在听的东西上：底部迷你条是一块被抬起的圆角石；Now Playing 从封面色里渗出一层暖水面。品牌名里的「澄」是透亮与安静，「波」既是水纹也是电波。气质是温暖的伴听，不是冷工具说明书，也不是舞台灯光秀。

**Brand slogan: 静水之上，波声不息** — 与 Creative North Star 一脉相承；与 PRODUCT.md 的 Brand Commitments 中的 tagline（听国内广播）并列，不要互相替换。Slogan 已落地于关于页；Splash 与官网首屏尚未实现。

密度跟一份认真的 Material 3 应用走。目的地是可扫的列表：顶上搜索，下面横滑分类与码率文字标签栏（主色下划线），列表行间 1px 分割。设置 → 外观可开「紧凑列表」（默认关）：只给电台行和单集行设 `ListTile` compact，不改全局 `ThemeData.visualDensity`，底栏和芯片保持标准。设置仍是同一套 ListTile，用主色分组标题切开「关于与外观 / 存储 / 播客 / 电台管理 / 分类」；「电台管理」里检测与刷新是带 TextButton 的操作行，不是另一套控制台。个性只靠三件事：种子色生成的蓝调表面、无台标时的二字渐变封面、以及那条始终在听的迷你条。深色模式是一等公民，由同一颗种子生成，不是浅色取反。
...
### Secondary
- **灰蓝 secondary**：FilterChip 选中容器、NavigationBar 指示。比主色更静，用来标记「当前筛选 / 当前目的地」，不是「正在播放」。电台页顶部分类与码率标签沿用「收听」页 TabBar 的主色下划线选中态。
...
列表统一为 `ListView` + `Divider(height: 1)`。电台页：顶上搜索，下面是可横滑的分类与码率文字标签栏，选中项走主色下划线。收藏页用 `titleLarge` 分区头，收藏与最近之间用间距 + 分割线分开。统计页展示总时长卡片、电台/播客占比条、最常收听列表；设置页用主色 `labelLarge` 分组标题（首组上 16，其后上 24，下 8）；分组顺序：播客管理 → 电台管理 → 播放与收听 → 外观 → 数据管理 → 电台分类 → 关于。「电台管理」组在标题下用一行 `bodySmall` 说明检测与刷新的差别，检测/刷新与后面的开关之间用一条分割线。
...
**The Shell Holds the Player Rule.** 播放器占位由 HomeShell 负责。目的地列表只留 16px 底边。空状态与**尚无台时的**探测进度都是居中短句加一句说明，需要时给一个主操作，不要插画英雄区。探测中已有台时改顶上紧凑横条（进度 +「停止检测」），列表可先点播。
...
### Chips
- **FilterChip:** 播客单集筛选与收听范围编辑中的类型/省份选择。未选走表面，选中走 `secondary-container`（静的灰蓝，不是主色高亮）。
...
### Mini player（签名）
底部圆角石。`surface-container-high`、半径 12、elevation 6。左 48 封面，中台名 `titleSmall` w600 + 副文 `bodySmall`，右：睡眠倒计时（主色）→ 播放/暂停 Filled → 停止。直播副文是 ICY 曲名或分类；曲名一行放不下才横向慢滚（约 6s + 溢出/40），分类不滚。直播时底边 3px 主色条；播客改为 3px 线性进度。无当前播放时整条消失（`SizedBox.shrink`），不要占位空壳。冷启动若「记住上次收听」打开，迷你条回填上次电台/单集，处于暂停，点播放才出声。Android 完整窗：仅封面+标题区可左右滑（电台切台、播客 ± 秒）；播放/停止键不跟手势。桌面迷你窗只拖窗口，不滑切台。
...
全屏模态，不可返回跳过。AppBar 标题「选择想听的电台」，无返回键。正文 `bodyMedium` variant 说明须至少选一项。区块：**加载全部精选** SwitchListTile；**类型** 横滑 FilterChip（央广 / 音乐 / 新闻 / 交通 / 财经 / 生活 / 文艺 / 综合），右上 TextButton「仅央广」；**省份 / 地区** Wrap FilterChip（34 区），右上「清空 / 全选」。底部 SafeArea 内 FilledButton「开始检测并进入」。确认后进电台页：尚无台时居中进度，测到后顶上紧凑横条 + 列表可先听，均可「停止检测」。选中态走 `FilterChip` 默认 selected 样式（与播客筛选芯片同一词汇）。设置里复用同一页，标题改「收听范围」，按钮改「保存并重新检测」，带返回键。
...
- **Do** 正在收听的电台/单集用 `primaryContainer` 选中底，不要用分类芯片的 secondary；leading 只叠静态播放图标。
...
- **Don't** 用 `positionStream` 做频谱/波形；正在播放只用选中底 + 静态图标。
- **Don't** 把 `ThemeData.visualDensity` 改成 compact 来压列表（会压底栏和芯片）；紧凑只设在 ListTile 上。
- **Don't** 在 Windows 列表或 `DeskMiniBar` 上做滑动手势。
