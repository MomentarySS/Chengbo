---
name: 澄波
description: 听国内广播 — Material 3 列表与 Spotify 式迷你播放条（对应产品 1.2.3）
colors:
  chengbo-blue: "#1565C0"
  deep-chengbo: "#0D4F8C"
  primary: "#405F90"
...
对应产品版本 **1.4.7**。底部导航「收藏」已改为「收听」三段式 tab（收藏 / 最近 / 统计）；播放器控制行改为 `spaceEvenly`；桌面迷你窗支持全桌面平台；收听统计页展示总时长、电台 vs 播客占比、最常收听 Top 5；外观切换带 200ms AnimatedTheme 过渡；Chromecast 投屏默认关闭，设置「外观」页可手动开启；播客主页顶部展示「继续收听」卡片，订阅列表项改为长按菜单；设置页「源」改名为「电台管理」；Now Playing 顶部标题精简；桌面迷你窗修复无边框窗口下首次点击穿透问题。
...
- **ActionChip:** 睡眠定时预设（15/30/45/60）、「自定义」与「本集结束」。选时长或本集即关 sheet。
...
### Desk mini window（签名）
仅桌面。无系统标题栏，窗口贴着一条 456×100 的浮条。条本身 64 高、半径 32、`surface` 底、阴影 `0 8 22`（`shadow` @ 22%）。左侧 72 圆封面探出条外，带 2px `outline-variant` 描边。条内：台名 + 副文，然后收藏、上一台/下一台（播客为 ±15 秒）、正圆主播放 52、× 回到完整窗口。可拖动整条。颜色走当前 ColorScheme，不要抄别的播放器绿。未在播放时仍显示浮条（「未在播放」），不要留一块空窗。
...
铺满屏幕，无顶圆角卡片感。背景：封面主色 55% 叠 `surface`，在 45% 高度淡出。大封面最大 340，放在 `Expanded` 里，标题变长时封面变矮，控制键钉在底部。标题 headlineSmall 加粗居中，最多 5 行。副标题同样：ICY 过长跑马灯，否则分类。音量在半径 16 的半透明槽里，轨道 4px、滑块半径 7。控制键用 `MainAxisAlignment.spaceEvenly` 均分间距，按钮保持固定大小不压缩：Outlined 返回 | 停止 | 圆形主播放 72 | 睡眠 | 播放列表。Outlined 固定 48 正圆，主播放 72 正圆。宽窗保持设计尺寸居中，窄窗整排等比缩小，不要把返回/播放列表钉在两侧单独拉伸。电台另有上一台 / 下一台。播客在进度条下有倍速与简介芯片，主播放两侧是 −15 / +15。播放列表弹出当前队列：电台为筛选/收藏/可见台，播客为这档节目的单集。Android 再跟一个 Outlined 投屏。投屏只在 Android 出现，不要为 Windows 做假按钮。
