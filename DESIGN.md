---
name: 澄波
description: 听国内广播 — Material 3 列表与 Spotify 式迷你播放条（对应产品 2.0.2）
colors:
  chengbo-blue: "#1565C0"
  deep-chengbo: "#0D4F8C"
  primary: "#405F90"
...
全屏模态，不可返回跳过。AppBar 标题「选择想听的电台」，无返回键。正文 `bodyMedium` variant 说明须至少选一项。区块：**加载全部精选** SwitchListTile；**类型** 横滑 FilterChip（央广 / 音乐 / 新闻 / 交通 / 财经 / 生活 / 文艺 / 综合），右上 TextButton「仅央广」；**省份 / 地区** Wrap FilterChip（34 区），右上「清空 / 全选」。底部 SafeArea 内 FilledButton「开始检测并进入」。确认后进电台页：尚无台时居中进度，测到后顶上紧凑横条 + 列表可先听，均可「停止检测」。选中态走 `FilterChip` 默认 selected 样式（与电台页分类芯片同一词汇）。设置里复用同一页，标题改「收听范围」，按钮改「保存并重新检测」，带返回键。
